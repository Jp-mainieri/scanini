/**
 * Scanini — Cloudflare Pages Function (catch-all /api/*)
 * Mesma lógica do Worker standalone, sem rota externa necessária.
 */
...
async function handleScan(request, env) {
  ...
  if (!parsed) return json({ error: 'IA retornou resposta inválida' }, 502);
...
async function handleWebhookMP(request, env) {
  ...
  if (type !== 'payment') return json({ ok: true, skipped: true });
  ...
  if (payment.status !== 'approved') return json({ ok: true, skipped: true });
  ...
  console.log(`Código gerado: ${code} para pagamento ${paymentId}`);
...
function validateMPSignature(signature) {
